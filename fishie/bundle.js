...
function IsWxGameEnv() {
  return WX_GAME_ENV;
}
function IsWxGameDevTools() {
  return WX_GAME_DEVTOOLS;
}
...
var wxhelper = {
  IsWxGameEnv: IsWxGameEnv,
  IsWxGameDevTools: IsWxGameDevTools,
  Now: Now,
  CreateImage: CreateImage,
  GetMainCanvas: GetMainCanvas,
  GetWindowSize: GetWindowSize,
  GetWindowSizeInPx: GetWindowSizeInPx,
  GetCanvasSizeUseWindowRatio: GetCanvasSizeUseWindowRatio,
  TimeUtil: TimeUtil,
  FPSMeter: FPSMeter
};
...
var ctx3;
// var startFish = 100;         //number of fish to start with
...
var assets = {
    background: "background-flip2.png",
    fish: "fishstrip.png"
};
...
function Fish() {
    ...
    function swim() {
        ...
        // ctx.test(imageStrip, x, y, flip * scale, 0, 0, scale, 0, 0, fishW * cell, fishH * species, fishW, fishH, -fishW / 2, -fishH / 2, fishW, fishH);
...
function init() {
    ...
    console.log("FishIE Tank init, canvas " + canvas.width + "x" + canvas.height + ", fish number:" + startFish);
}
...
function loop() {
    ...
    // ctx.font = "50px Arial";
    // ctx.fillStyle = "white"
    // ctx.fillText("FishIE Tank framerate:" + fpsMeter.getFramerate().toFixed(0) + "fps", 10, 200);
    // setTimeout(loop, 16);
